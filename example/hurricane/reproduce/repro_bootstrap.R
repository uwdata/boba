#!/usr/bin/env Rscript
# Use bootstrapping to get uncertainty distribution
# Issue: majority of glm.nb models fail to fit on bootstrapped data
# --- (BOBA_CONFIG)
{
  "decisions": [
    ...
    {"var": "model_prefix", "options": ["lm(log_death", "glm.nb(death"]},
    {"var": "df", "options": [
        "pred$df",
        "df.residual(model)"
    ]}
  ...
  "constraints": [
    {"link": ["Model", "back_transform", "df", "model_prefix"]}
  ],
...
rsq <- function(data, indices) {
  ...
  model <- {{model_prefix}} ~ {{predictors}} {{covariates}} , data = d)
  pred <- predict(model, se.fit = TRUE, type = "response")
  disagg_fit <- d  %>%
    mutate(
        fit = pred$fit,                            # add inferential fits and standard errors to dataframe
  ...
  expectation <- disagg_fit %>%
    ...
    compare_levels(expected_deaths, by = female) %>%
    ungroup()
  return(expectation$expected_deaths)
}

# bootstrap
bootstrap <- boot(data=df, statistic=rsq, R=200)
bootstrap <- tidy(bootstrap$t) %>%
  select(expected_diff=x)
...
# visualize
library(ggplot2)
ggsave(qplot(expected_diff, data=bootstrap, geom="histogram"), file='../results/bootstrap_{{_n}}.pdf')
ggsave(qplot(expected_diff, data=uncertainty, geom="histogram"), file='../results/uncertainty_{{_n}}.pdf')
...
write_csv(bootstrap, '../results/bootstrap_{{_n}}.csv')
